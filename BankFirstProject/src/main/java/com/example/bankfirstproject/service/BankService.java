...
import com.example.bankfirstproject.repository.TransactionRepository;
...
public class BankService implements BankServiceRepo {
	...
	public String fundtransfer(TransferDTO transferdto) {
		...
		else
		{
			...
			trans.setFromAccount(bank1.get().getId());
			trans.setToAccount(bank2.get().getId());
			trans.setTransactedAmount(transferdto.getAmount());
